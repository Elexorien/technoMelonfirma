...
public class ItemTechnofirmaIngot extends ItemTerra implements ISmeltable
{
	public String Icon;
	...
	@Override
	public ItemTechnofirmaIngot setSize(EnumSize s)
	{
		size = s;
		return this;
	}
	...
	private boolean CreatePile(ItemStack itemstack, EntityPlayer entityplayer, World world, int x, int y, int z, int side, int l)
	{
		...
		TETFIngotPile te = null;

		if (world.getTileEntity(x, y, z) instanceof TETFIngotPile && world.getBlock(x,y,z) == TechnoItems.TFIngotPile)
		{
			te = (TETFIngotPile)world.getTileEntity(x, y, z);
		...
		if(fullStack && isPlaceable(itemstack))
		{
			...
			{
				world.setBlock( x, y-1, z, TechnoItems.TFIngotPile, l, 0x2);
				if(world.isRemote) {
					world.markBlockForUpdate(x, y-1, z);
				}
				te = (TETFIngotPile)world.getTileEntity(x, y-1, z);
			}
			...
			{
				world.setBlock( x, y, z-1, TechnoItems.TFIngotPile, l, 0x2);
				if(world.isRemote) {
					world.markBlockForUpdate(x, y, z-1);
				}
				te = (TETFIngotPile)world.getTileEntity(x, y, z-1);
			}
			...
			{
				world.setBlock( x-1, y, z, TechnoItems.TFIngotPile, l, 0x2);
				if(world.isRemote) {
					world.markBlockForUpdate(x-1, y, z);
				}
				te = (TETFIngotPile)world.getTileEntity(x-1, y, z);
			}
	...
	public boolean onItemUse(ItemStack itemstack, EntityPlayer entityplayer, World world, int x, int y, int z, int side, float hitX, float hitY, float hitZ)
	{
		...
		if(entityplayer.isSneaking() && stackTagCompound == null && (itemstack.getItem().getUnlocalizedName().indexOf("Double")==-1) && this.isPlaceable(itemstack))
		{
			...
			if(!world.isRemote && entityplayer.isSneaking() && (world.getBlock(x, y, z) != TechnoItems.TFIngotPile || (side != 1 && side != 0)))
			{
				...
				if(CreatePile(itemstack, entityplayer, world, x, y, z, side, dir))
				{
					...
					world.addBlockEvent(x,y,z,TechnoItems.TFIngotPile,0,0);
					return true;

				}
			}
			else if(world.getBlock(x, y, z) == TechnoItems.TFIngotPile)
			{
				TETFIngotPile te = (TETFIngotPile)world.getTileEntity(x, y, z);
			...
			else
			{
				...
				{
					if (m>=16){
						world.setBlock(x, y+1, z, TechnoItems.TFIngotPile, m, 0x2);
						itemstack.stackSize = itemstack.stackSize-1;
					}
					else{
						world.setBlock(x, y+1, z, TechnoItems.TFIngotPile, m, 0x2);
				...
				if (world.getTileEntity(x,y,z) != null && world.getTileEntity(x,y,z) instanceof TETFIngotPile)
				{
	...
	{
		if(world.isSideSolid(i, j-1, k, ForgeDirection.UP) || world.getBlock(i, j-1, k)==TechnoItems.TFIngotPile)
		{
			TileEntity te = world.getTileEntity(i, j-1, k);

			if (te instanceof TETFIngotPile)
			{
				TETFIngotPile ip = (TETFIngotPile)te;
	...
	public void setSide(World world, ItemStack itemstack, int m, int dir, int x, int y, int z, int i, int j, int k)
	{
		...
		{
			if(dir == 0 || dir == 2) {
				world.setBlock(x+i, y+j, z+k, TechnoItems.TFIngotPile, m, 0x2);
			} else {
				world.setBlock(x+i, y+j, z+k, TechnoItems.TFIngotPile, m | 8, 0x2);
			}
			itemstack.stackSize = itemstack.stackSize-1;
		}
		else if(m >= 16)
		{
			if(dir == 0 || dir == 2) {
				...
				world.setBlock(x+i, y+j, z+k, TechnoItems.TFIngotPile, m-8 | 8, 0x2);
			}
			itemstack.stackSize = itemstack.stackSize-1;
		}
