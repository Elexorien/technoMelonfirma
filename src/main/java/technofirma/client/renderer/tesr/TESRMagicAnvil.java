...
public class TESRMagicAnvil extends TESRBase
{
	public TESRMagicAnvil()
	{
		super();
	}
	 ...
	 */
	public void renderAt(TEMagicAnvil te, double d, double d1, double d2, float f)
	{
		if (te.getWorldObj() != null)
		{
			...
			drawItem(te, d, d1, d2, dir, customitem, blockScale, TEMagicAnvil.HAMMER_SLOT);
			drawItem(te, d, d1, d2, dir, customitem, blockScale, TEMagicAnvil.INPUT1_SLOT);
			drawItem(te, d, d1, d2, dir, customitem, blockScale, TEMagicAnvil.WAND_SLOT);
			//drawItem(te, d, d1, d2, dir, customitem, blockScale, te.FLUX_SLOT);
		}
	}

	private void drawItem(TEMagicAnvil te, double d, double d1, double d2, int dir, EntityItem customitem, float blockScale, int i)
	{
	...
	public float[] getLocation(int dir, int slot, boolean isStone)
	{
		...
		if(dir == 0)
		{
			...
			out[5] = 0f;
			out[6] = 0f;
			if(slot == TEMagicAnvil.HAMMER_SLOT)
			{
				out[0] = 0.55f;
				out[1] = 0.61f;
				out[2] = 0.45f;
			}
			else if(slot == TEMagicAnvil.INPUT1_SLOT)
			{
				out[0] = 0.55f;
				out[1] = 0.61f;
				out[2] = 0.05f;
			...
			else if(slot == TEMagicAnvil.WAND_SLOT)
			{
				out[0] = 0.7f;
				out[1] = 0.61f;
				out[2] = 0.35f;
	...
	{
		this.renderAt((TEMagicAnvil)te, xDis, yDis, zDis, f);
	}
}
