...
public class BlockTFIngotPile extends BlockIngotPile
{
	private Random random = new Random();

	public BlockTFIngotPile()
	{
		super();
	}
	...
	public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer entityplayer, int side, float hitX, float hitY, float hitZ)
	{
		...
		{
			if((TETFIngotPile)world.getTileEntity(x, y, z) != null)
			{
				TETFIngotPile tileentityingotpile;
				tileentityingotpile = (TETFIngotPile)world.getTileEntity(x, y, z);
	...
	{
		TETFIngotPile teip = (TETFIngotPile)world.getTileEntity(x, y, z);
		TETFIngotPile teipBottom = (TETFIngotPile)world.getTileEntity(x, y - 1, z);
	...
	{
		TETFIngotPile teip = (TETFIngotPile)world.getTileEntity(x, y + 1, z);
		TETFIngotPile teipBottom = (TETFIngotPile)world.getTileEntity(x, y, z);
	...
	public void setBlockBoundsBasedOnState(IBlockAccess bAccess, int x, int y, int z)
	{
		...
		//int direction = getDirectionFromMetadata(meta);
		TETFIngotPile te = (TETFIngotPile)bAccess.getTileEntity(x, y, z);
	...
	public int getStack(World world,TETFIngotPile tt)
	{
		TETFIngotPile Te = ((TETFIngotPile)world.getTileEntity(tt.xCoord, tt.yCoord, tt.zCoord));
	...
	public void onBlockPreDestroy(World world, int i, int j, int k, int meta) 
	{
		if(!world.isRemote)
		{
			TETFIngotPile te = (TETFIngotPile)world.getTileEntity(i, j, k);
			if(te != null && te.getStackInSlot(0) != null)
			{
	...
	public void onNeighborBlockChange(World world, int x, int y, int z, Block block)
	{
		if(!world.isRemote)
		{
			...
			{
				if(world.getBlock(x, y - 1, z) == this && ((TETFIngotPile)world.getTileEntity(x, y, z)).storage[0].getItem() == ((TETFIngotPile)world.getTileEntity(x, y - 1, z)).storage[0].getItem())
				{
					combineIngotsDown(world, x, y, z);
				}
				else if(world.getBlock(x, y + 1, z) == this && ((TETFIngotPile)world.getTileEntity(x, y, z)).storage[0].getItem() == ((TETFIngotPile)world.getTileEntity(x, y + 1, z)).storage[0].getItem())
				{
					combineIngotsUp(world, x, y, z);
				}
				else
				{
					((TETFIngotPile)world.getTileEntity(x, y, z)).ejectContents();
					world.setBlockToAir(x, y, z);
					return;
				}
			}
